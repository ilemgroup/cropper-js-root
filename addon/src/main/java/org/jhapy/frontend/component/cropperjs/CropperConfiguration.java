...
public class CropperConfiguration {
    ...
    public String getJsonString() {
        ...
        if (data != null) {
            parameters.add(String.format(Locale.ROOT, "\"data\": %s", data.getJsonString()));
        }

        if (aspectRatio != null) {
            parameters.add(String.format(Locale.ROOT, "\"aspectRatio\": %.2f", aspectRatio));
        }

        if (autoCrop != null) {
            parameters.add(String.format(Locale.ROOT, "\"autoCrop\": %s", autoCrop));
        }

        if (autoCropArea != null) {
            parameters.add(String.format(Locale.ROOT, "\"autoCropArea\": %.2f", autoCropArea));
        }

        if (background != null) {
            parameters.add(String.format(Locale.ROOT, "\"background\": %s", background));
        }

        if (center != null) {
            parameters.add(String.format(Locale.ROOT, "\"center\": %s", center));
        }
        ...
        if (cropBoxMovable != null) {
            parameters.add(String.format(Locale.ROOT, "\"cropBoxMovable\": %s", cropBoxMovable));
        }
        ...
        if (dragMode != null) {
            parameters.add(String.format(Locale.ROOT, "\"dragMode\": \"%s\"", dragMode.getKey()));
        }

        if (guides != null) {
            parameters.add(String.format(Locale.ROOT, "\"guides\": %s", guides));
        }

        if (highlight != null) {
            parameters.add(String.format(Locale.ROOT, "\"highlight\": %s", highlight));
        }
        ...
        if (modal != null) {
            parameters.add(String.format(Locale.ROOT, "\"modal\": %s", modal));
        }

        if (movable != null) {
            parameters.add(String.format(Locale.ROOT, "\"movable\": %s", movable));
        }

        if (responsive != null) {
            parameters.add(String.format(Locale.ROOT, "\"responsive\": %s", responsive));
        }

        if (restore != null) {
            parameters.add(String.format(Locale.ROOT, "\"restore\": %s", restore));
        }

        if (rotatable != null) {
            parameters.add(String.format(Locale.ROOT, "\"rotatable\": %s", rotatable));
        }

        if (scalable != null) {
            parameters.add(String.format(Locale.ROOT, "\"scalable\": %s", scalable));
        }
        ...
        if (viewMode != null) {
            parameters.add(String.format(Locale.ROOT, "\"viewMode\": %d", viewMode.getKey()));
        }
        ...
        if (zoomOnTouch != null) {
            parameters.add(String.format(Locale.ROOT, "\"zoomOnTouch\": %s", zoomOnTouch));
        }

        if (zoomOnWheel != null) {
            parameters.add(String.format(Locale.ROOT, "\"zoomOnWheel\": %s", zoomOnWheel));
        }

        if (zoomable != null) {
            parameters.add(String.format(Locale.ROOT, "\"zoomable\": %s", zoomable));
        }
